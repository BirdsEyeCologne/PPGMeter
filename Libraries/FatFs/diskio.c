/*-----------------------------------------------------------------------*/
/* Low level disk I/O module skeleton for FatFs     (C)ChaN, 2013        */
/*-----------------------------------------------------------------------*/
/* If a working storage control module is available, it should be        */
/* attached to the FatFs via a glue function rather than modifying it.   */
/* This is an example of glue functions to attach various exsisting      */
/* storage control module to the FatFs module with a defined API.        */
/*-----------------------------------------------------------------------*/
/* Angepasst von UB                                                      */
/* V:1.0 / 26.02.2013                                                    */
/*-----------------------------------------------------------------------*/

#include "driver/stm32_sdio.h"	// UB: MMC/SDC control
#include "diskio.h"		                    // FatFs lower layer API

/* Definitions of physical drive number for each media */
#define MMC		0
#define USB		1
#define ATA		2

/*-----------------------------------------------------------------------*/
/* Inidialize a Drive                                                    */
/*-----------------------------------------------------------------------*/

DSTATUS disk_initialize(BYTE pdrv /* Physical drive nmuber (0..) */
) {
   DSTATUS stat;
   int result;

   switch (pdrv) {
      case ATA:
         stat = RES_PARERR;
         return stat;
      case MMC:
         result = MMC_disk_initialize();

         // translate the reslut code here
         if (result == 0) {
            stat = 0;
         } else {
            stat = STA_NOINIT;
         }

         return stat;

      case USB:
         stat = RES_PARERR;
         return stat;
   }
   return STA_NOINIT;
}

/*-----------------------------------------------------------------------*/
/* Get Disk Status                                                       */
/*-----------------------------------------------------------------------*/

DSTATUS disk_status(BYTE pdrv /* Physical drive nmuber (0..) */
) {
   DSTATUS stat;
   int result;

   switch (pdrv) {
      case ATA:
         stat = RES_PARERR;
         return stat;
      case MMC:
         result = MMC_disk_status();

         // translate the reslut code here
         if (result == 0) {
            stat = 0;
         } else {
            stat = STA_NODISK | STA_NOINIT;
         }

         return stat;

      case USB:
         stat = RES_PARERR;
         return stat;
   }
   return STA_NOINIT;
}

/*-----------------------------------------------------------------------*/
/* Read Sector(s)                                                        */
/*-----------------------------------------------------------------------*/

DRESULT disk_read(BYTE pdrv, /* Physical drive nmuber (0..) */
BYTE *buff, /* Data buffer to store read data */
DWORD sector, /* Sector address (LBA) */
UINT count /* Number of sectors to read (1..128) */
) {
   DRESULT stat;
   int result;

   switch (pdrv) {
      case ATA:
         stat = RES_PARERR;
         return stat;
      case MMC:
         // translate the arguments here

         result = MMC_disk_read(buff, sector, count);

         // translate the reslut code here
         if (result == 0) {
            stat = RES_OK;
         } else {
            stat = RES_ERROR;
         }

         return stat;

      case USB:
         stat = RES_PARERR;
         return stat;
   }
   return RES_PARERR;
}

/*-----------------------------------------------------------------------*/
/* Write Sector(s)                                                       */
/*-----------------------------------------------------------------------*/

#if _USE_WRITE
DRESULT disk_write(BYTE pdrv, /* Physical drive nmuber (0..) */
const BYTE *buff, /* Data to be written */
DWORD sector, /* Sector address (LBA) */
UINT count /* Number of sectors to write (1..128) */
) {
   DRESULT stat;
   int result;

   switch (pdrv) {
      case ATA:
         stat = RES_PARERR;
         return stat;
      case MMC:
         // translate the arguments here

         result = MMC_disk_write(buff, sector, count);

         // translate the reslut code here
         if (result == 0) {
            stat = RES_OK;
         } else {
            stat = RES_ERROR;
         }

         return stat;

      case USB:
         stat = RES_PARERR;
         return stat;
   }
   return RES_PARERR;
}
#endif

/*-----------------------------------------------------------------------*/
/* Miscellaneous Functions                                               */
/*-----------------------------------------------------------------------*/
DWORD get_fattime(void) {

   RTC_DateTypeDef today;
   RTC_TimeTypeDef now;

   RTC_GetTime(RTC_Format_BIN, &now);
   RTC_GetDate(RTC_Format_BIN, &today);

   DWORD date_time;

   date_time = ((uint32_t)today.RTC_Year + 2000 - 1980) << 25;
   date_time |= (uint32_t)today.RTC_Month << 21;
   date_time |= (uint32_t)today.RTC_Date << 16; // day of the month.
   date_time |= (uint32_t)now.RTC_Hours << 11;
   date_time |= (uint32_t)now.RTC_Minutes << 5;
   date_time |= (uint32_t)now.RTC_Seconds >> 1; // yes, division by 2 is right...

   return date_time;
}

#if _USE_IOCTL
DRESULT disk_ioctl(BYTE pdrv, /* Physical drive nmuber (0..) */
BYTE cmd, /* Control code */
void *buff /* Buffer to send/receive control data */
) {
   DRESULT stat;
   int result;

   switch (pdrv) {
      case ATA:
         stat = RES_PARERR;
         return stat;
      case MMC:
         // pre-process here

         result = MMC_disk_ioctl(cmd, buff);

         // post-process here
         if (result == 0) {
            stat = RES_OK;
         } else {
            stat = RES_ERROR;
         }

         return stat;

      case USB:
         stat = RES_PARERR;
         return stat;
   }
   return RES_PARERR;
}
#endif
