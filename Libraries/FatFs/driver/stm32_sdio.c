...
#if USE_MULTIBLOCK_MODE==1
  static DWORD  scratch[512/4];
#endif
...
//--------------------------------------------------------------
// READ-Funktion (SingleBlockMode)
// Return Wert :
//    0 = alles ok
//  < 0 = Fehler
//--------------------------------------------------------------
#if USE_MULTIBLOCK_MODE==0
...
int MMC_disk_read(BYTE *buff, DWORD sector, BYTE count)
{
  ...
  if((DWORD)buff & 3) {
    while (count--) {
      SD_ReadBlock((BYTE *)scratch,(uint64_t)((sector + count) << 9), 512);
      /* Check if the Transfer is finished */
      status =  SD_WaitReadOperation();
      while(SD_GetStatus() != SD_TRANSFER_OK);

      memcpy(&buff[count << 9] ,scratch, 512);
    }
  }
  else {
    SD_ReadMultiBlocks(buff, (uint64_t)(sector << 9), 512, count);
    /* Check if the Transfer is finished */
    status =  SD_WaitReadOperation();
    while(SD_GetStatus() != SD_TRANSFER_OK);
  }
...
//--------------------------------------------------------------
// WRITE-Funktion (SingleBlockMode)
// Return Wert :
//    0 = alles ok
//  < 0 = Fehler
//--------------------------------------------------------------
...
int MMC_disk_write(const BYTE *buff, DWORD sector, BYTE count)
{
  ...
  if((DWORD)buff & 3) {
    while (count--) {
      memcpy(scratch, &buff[count << 9], 512);

      SD_WriteBlock((BYTE *)buff, (uint64_t)((sector + count) << 9), 512);
      /* Check if the Transfer is finished */
      status = SD_WaitWriteOperation();
      while(SD_GetStatus() != SD_TRANSFER_OK);
    }
    ...
    SD_WriteMultiBlocks((BYTE *)buff, (uint64_t)(sector << 9), 512, count);
